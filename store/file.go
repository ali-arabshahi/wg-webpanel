...
// newFileStore : create new store for wireguard server and clients config in json format
func newFileStore(dataPath string, logger ILogger) (*fileStore, error) {
...
func (cs *fileStore) GetServerConfig() wgmgmt.Server {
	return cs.wgConfigStore.ServerConfig
}

//---------------------------------------------
func (cs *fileStore) SaveServerConfig(serverConf wgmgmt.Server) error {
...
//---------------------------------------------
func (cs *fileStore) AllClient() []wgmgmt.Client {
...
//---------------------------------------------
func (cs *fileStore) ClientByID(clientID int64) (wgmgmt.Client, error) {
...
//---------------------------------------------
func (cs *fileStore) AddClient(newClient wgmgmt.Client) error {
...
//---------------------------------------------
func (cs *fileStore) UpdateClient(updateClient wgmgmt.Client) error {
	found := false
...
//---------------------------------------------
func (cs *fileStore) RemoveClient(rmClient wgmgmt.Client) error {
	found := false
	for index, client := range cs.wgConfigStore.Clients {
...
//---------------------------------------------
func (cs *fileStore) AvailablePrivateIPS() []string {
...
//---------------------------------------------
func (cs *fileStore) UsedPrivateIPS() []string {
	usedIp := []string{}
	srvConfig := cs.wgConfigStore.ServerConfig
	serverIPRange := srvConfig.TunnelAddress
...
//---------------------------------------------
func (cs *fileStore) WriteSourceGroup(sourceIps map[string]*trafficcapture.SourceGroup) {
...
//---------------------------------------------
func (cs *fileStore) ClientTrafficByIP(clientIP string) (int64, int64, error) {
...
//---------------------------------------------
func (cs *fileStore) AllTraffic() (int64, int64, error) {
...
//---------------------------------------------
func (cs *fileStore) ClientNumber() (int, int) {
	cs.internalTableLock.Lock()
	defer cs.internalTableLock.Unlock()
	clients := cs.wgConfigStore.Clients
...
func (cs *fileStore) CheckUserAndPassword(adminUser usermanagment.AdminUser) bool {
...
//***********************************************//
func (cs *fileStore) saveWgDataToFile() error {
...
func (cs *fileStore) saveTrafficToFile() error {
