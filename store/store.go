...
type IStore interface {
	CheckUserAndPassword(adminUser usermanagment.AdminUser) bool
	GetServerConfig() wgmgmt.Server
	SaveServerConfig(wgmgmt.Server) error
	AllClient() []wgmgmt.Client
	ClientByID(clientID int64) (wgmgmt.Client, error)
	...
	AddClient(wgmgmt.Client) error
	UsedPrivateIPS() []string
	UpdateClient(wgmgmt.Client) error
	RemoveClient(wgmgmt.Client) error
	WriteSourceGroup(sourceIps map[string]*trafficcapture.SourceGroup)
}

//---------------------------------------------
var errClientNotFound = errors.New("client not found")

func NewStore(sType string, dataPath string, logger ILogger) (IStore, error) {
	var sErr error
	var storeInstance IStore
	if sType == FileType {
		store, err := newFileStore(dataPath, logger)
		if err != nil {
			storeInstance = nil
			sErr = err
		} else {
	...
	} else if sType == SqlliteType {
		store, err := newSqliteStore(dataPath, logger)
		if err != nil {
			storeInstance = nil
			sErr = err
		} else {
		...
		storeInstance = nil
		sErr = fmt.Errorf("database driver %v not dound", sType)
	}
	return storeInstance, sErr
}
